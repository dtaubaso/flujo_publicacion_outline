...
def main():
        ...
        for tab, kw in zip(tabs, keywords):
            with tab:
                ...
                st.session_state.analysis_results[kw] = {
                    ...
                    'auto': auto,
                    'config': config,
                    'full_outline_md': full_outline_md,
                }
    ...
    # NUEVO: Mostrar resultados guardados aunque run_btn sea False
    if st.session_state.analysis_results:
        keywords = list(st.session_state.analysis_results.keys())
        tabs = st.tabs([f"{k}" for k in keywords])
        for tab, kw in zip(tabs, keywords):
            with tab:
                data = st.session_state.analysis_results[kw]
                df = data['df']
                ...
                auto = data['auto']
                config = data['config']
                full_outline_md = data.get('full_outline_md', outline_md)
